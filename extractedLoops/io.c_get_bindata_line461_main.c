#include <io.c_get_bindata_line461_1.ptrderefout.h>
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <limits.h>
...
#include <io.c_get_bindata_line461_1.h>
#include <common.h>
#include <timer.h>
...
int main(int argc, char** argv)
{
    ...
    static char* inputDataFileName[] = {"io.c_get_bindata_line461_1.ptrderefout.hd", "io.c_get_bindata_line461_1.ptrderefout.st", "io.c_get_bindata_line461_1.ptrderefout.gl"};
    ...
    for (int it = 0; it < iterations; it++)
    {
        ...
        sptr = (struct fvec*) sptr_;


        /*----------------- read data for buf----------------------*/
        char buf_dataFileName[] = "/dataFile/io.c_get_bindata_line461_1.buf.dat";
        ...
        if ( it < 1 )
        {
            ...
            if (pFile_ == NULL)
            {
                printf("data file: /dataFile/io.c_get_bindata_line461_1.buf.dat does not exist!");
                exit(-1);
            }
            while(fscanf(pFile_, "%d", &inputIndices[0]) != EOF)
            {
                fscanf(pFile_,"%d", &short_storage);
                buf_[inputIndices[0]] = short_storage;
            }
            fclose(pFile_);
        }
        ...
        memcpy(&pptr->inswapbytes, &pptr_inswapbytes_, sizeof(pptr_inswapbytes_));
        sptr->values = (float *)sptr_values_;
    ...
    void* ptrCheckSumVal = dummy(2, pptr, sptr);
