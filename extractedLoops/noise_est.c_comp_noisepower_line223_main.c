#include <noise_est.c_comp_noisepower_line223_1.ptrderefout.h>
#include <stdio.h>
#include <string.h>
#include <stdlib.h>
#include <limits.h>
...
#include <noise_est.c_comp_noisepower_line223_1.h>
#include <common.h>
#include <timer.h>
...
struct svec* restrict new_max;
struct svec* restrict new_max_index;
struct fvec* restrict noise_level_lt;
struct svec* restrict histo_index;
int j;
int histo_length;
struct fmat
{
...
struct fmat* restrict histo_values;
...
int main(int argc, char** argv)
{
    ...
    static char* inputDataFileName[] = {"noise_est.c_comp_noisepower_line223_1.ptrderefout.hd", "noise_est.c_comp_noisepower_line223_1.ptrderefout.st", "noise_est.c_comp_noisepower_line223_1.ptrderefout.gl"};
    ...
    for (int it = 0; it < iterations; it++)
    {
        ...
        max = (struct fvec*) max_;
        new_max = (struct svec*) new_max_;
        new_max_index = (struct svec*) new_max_index_;
        noise_level_lt = (struct fvec*) noise_level_lt_;
        histo_index = (struct svec*) histo_index_;
        all_trajec = (struct fmat*) all_trajec_;
        histo_values = (struct fmat*) histo_values_;

        memcpy(&histo_length, &histo_length_, sizeof(histo_length_));
        ...
        memcpy(&pptr->nfilts, &pptr_nfilts_, sizeof(pptr_nfilts_));
        max->values = (float *)max_values_;
        new_max->values = (short *)new_max_values_;
        new_max_index->values = (short *)new_max_index_values_;
        noise_level_lt->values = (float *)noise_level_lt_values_;
        histo_index->values = (short *)histo_index_values_;
        all_trajec->values = (float **)all_trajec_values_;
        histo_values->values = (float **)histo_values_values_;
    ...
    void* ptrCheckSumVal = dummy(8, all_trajec, histo_index, histo_values, max, new_max, new_max_index, noise_level_lt, pptr);
